...
def ATL_BDD():
	...
	try:
		...
		if dt_BDD != dt_ANEEL:
			...
			try:
				...
				try:
					...
					wget.download(url_ATL, f"BDD/ANEEL/{dt_ANEEL}.csv")

					# Tratamento dos novos dados - Substituir ',' por '.'
					try:
						arq = open(f"BDD/ANEEL/{dt_ANEEL}.csv","rt")
						dados = arq.read()
						dados = dados.replace(",",".")
						arq.close()
						arq = open(f"BDD/ANEEL/{dt_ANEEL}.csv","wt")
						arq.write(dados)
						arq.close()
					...
					Título("Banco de Dados ATUALIZADO","+")
				except:
		...
		Título("ERRO - FALHA NA OBTENÇÃO DOS DADOS DA ANEEL","*")
